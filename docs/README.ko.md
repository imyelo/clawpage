...
> 📤 하나의 명령으로 OpenClaw 채팅을 아름답고 영구적인 페이지로 변환 — 자신의 GitHub Pages에 자동 배포

OpenClaw 대화 기록을 정적 웹사이트로 공유하기 위한 monorepo입니다. 채팅 채널에서 공유 명령을 실행하면 영구적으로 공개적으로 액세스 가능한 페이지를 얻을 수 있습니다 — GitHub Pages를 통해 자동으로 구축 및 배포됩니다.
...
agent는私有 GitHubrepo를 스캐폴딩하고 Pages URL로 `chats-share.toml`을 구성하고 초기 커밋을 푸시하고 GitHub Actions를 Pages 소스로 활성화하며 `/chats-share`가 즉시 작동하도록 프로젝트를 등록합니다. 전체 단계별 가이드는 [skills/chats-share/references/setup.md](skills/chats-share/references/setup.md)를 참조하세요.
...
이 저장소는 **공개 템플릿**입니다. 실제 채팅 데이터는 별도의 **私有 작업 저장소**에 저장됩니다 — 이렇게 하면 데이터 오염 없이 템플릿을 깨끗하고 포크 가능한 상태로 유지할 수 있습니다.

| 저장소 | 공개 여부 | 목적 |
|------|------------|---------|
| `openclaw-chats-share` | 공개 | 템플릿, 패키지 및 Skill |
| `your-chats-share` |私有 | 실제 채팅 데이터 |
...
| `visibility` | 아니요 | 인덱스可见성 | `private` (기본값) |
...
### 사용자 정의 도메인 (GitHub Pages)

`your-username.github.io` 대신 사용자 정의 도메인에서 사이트를 제공하려면:

1. `public/` 디렉터리에 도메인이 포함된 `CNAME` 파일 추가:

   ```
   chats-share.example.com
   ```

2. `chats-share.toml`에서 `site`을 사용자 정의 도메인으로 설정:

   ```toml
   site = "https://chats-share.example.com"
   ```

3. DNS 공급자에서 도메인이 GitHub Pages를 가리키도록 구성:
   - apex 도메인(`example.com`): GitHub IP를 가리키는 A 레코드 추가
   - 서브도메인(`chats-share.example.com`): `your-username.github.io`를 가리키는 CNAME 레코드 추가

4. DNS가 전파된 후 GitHub 저장소의 **Settings → Pages**에서 HTTPS 활성화.

> 사용자 정의 도메인을 사용할 때는 `chats-share.toml`에서 `base`를 생략(또는 `"/"`로 설정)하세요. 사이트가 도메인 루트에서 제공되기 때문입니다.

상세한 GitHub Pages 사용자 정의 도메인 설정 지침은 [Managing a custom domain for your GitHub Pages site](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/managing-a-custom-domain-for-your-github-pages-site)도 참조하세요.
...
-あなたのサイト — [PR을 제출](https://github.com/imyelo/openclaw-chats-share/edit/main/README.md)하여 추가하세요!
