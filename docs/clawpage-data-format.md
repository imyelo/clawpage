# Clawpage Data Format

This directory contains Openclaw session export files in YAML format, generated by the CLI from Openclaw session JSONL files (`{id}.jsonl`).
...
The web package reads `.yaml` files from the `chats/` directory (or `chats_dir` from `clawpage.toml`) and renders them as a conversation view. Each timeline item is rendered as follows:
